...
```text
src/ov_search_skill/retrievers/
  检索后端适配层

src/ov_search_skill/workflows/
  多步骤工作流

src/ov_search_skill/cli.py
  命令行入口
```
...
-> OpenViking 建索引
-> ov-search-skill search/research
```

不建议一开始做：

```text
ov-search-skill search 同时查 OpenViking 和 AnySearch
```
...
```text
src/ov_search_skill/connectors/
  __init__.py
  base.py
  anysearch.py
```
...
```powershell
ov-search-skill fetch-web "AI search tools" --provider anysearch --output data\web\ai-search
```

然后用户再运行：

```powershell
ov-search-skill import-local data\web\ai-search --to viking://resources/ai-search-web
```
...
- Bot 是可选体验层。
- 不影响 `ov-search-skill search`。
- 不影响 `ov-search-skill research`。
- 不要求普通用户必须安装 Bot 依赖。
...
ov-search-skill research examples\synthetic_ai_news\research_questions.yaml --output reports\synthetic_ai_news_research_draft.md --json-output reports\synthetic_ai_news_research_draft.json --top-k 4
